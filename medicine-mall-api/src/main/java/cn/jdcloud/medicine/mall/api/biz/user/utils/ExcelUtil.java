...
public class ExcelUtil {
    ...
    public static <T> void exportMultisheetExcel(String name, List<Map> mapList, HttpServletResponse response) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new ApiException(UserCode.EXPORT_ERROR);
        } finally {
            try {
                if (bos != null) {
                    bos.close();
     ...
     * @return
     */
    public static List<Map<String, String>> readExcel(MultipartFile file, Integer sheetIndex) throws IOException {
    ...
    private static <T> void createSheet(SXSSFWorkbook wb, String[] exportFields, String[] headers, Collection<T> dataList, String fileName, int maxBuff) throws NoSuchFieldException, IllegalAccessException, IOException {
        ...
        for (int cellnum = 0; cellnum < headerSize; cellnum++) {
            ...
            sh.setColumnWidth(cellnum, 6000);
    ...
    private static Workbook createWorkBook(MultipartFile file) throws IOException {
        ...
        if(!validateExcel(file.getOriginalFilename())){
            throw new ApiException(UserCode.FILE_FORMAT_ERROR);
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new ApiException(UserCode.NOTFINDFILE_ERROR);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return wb;
