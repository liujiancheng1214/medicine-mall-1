...
window.clusterizeTable=Vue.component('clusterize-table',{
    created(){
    ...
    methods:{
        /*计算表格col和主体滚动部clusterizeTable分高度*/
        ...
        resetHtml(obj,index,key){
            let tdIndex=null;
            for(let j=0;j<vm.excleTableHead.length;j++){
                if(vm.excleTableHead[j].title==key){
                    tdIndex=j;
                    break;
                }
            }
            var sex=document.querySelector(`#contentArea tr[data-index="${index}"]>td[data-tdIndex="${tdIndex}"]>div`).innerHTML;
            vm.excleTableData[index][key]=obj.value;
            console.log(vm.excleTableData[index]);
            obj.parentNode.innerHTML=obj.value;
            var re =new RegExp(`data-tdIndex="${tdIndex}"><div class="ivu-table-cell">.*</div></td>(<td style="cursor:pointer" data-tdIndex="${tdIndex+1}">){1}`,'');
            var re2=new RegExp(`data-tdIndex="${tdIndex}"><div class="ivu-table-cell">.*</div></td><td style="cursor:pointer" data-tdIndex="${tdIndex+1}">`);
            var T=re2.test(clusterize.rows[index])
            clusterize.rows[index]=clusterize.rows[index].replace(re,(`data-tdIndex="${tdIndex}"><div class="ivu-table-cell">${obj.value}</div></td>`+(T?`<td style="cursor:pointer" data-tdIndex="${tdIndex+1}">`:``)));
                console.log(clusterize.rows[index]);
            },
        /*初始化table*/
        initTable(scrollId,contentId){
            ...
            for(let i=0;i<this.data.length;i++){
                ...
                for(let j in this.head){
                    if(j==0 && i==0){
                        str+=`<td style="cursor:pointer" data-tdIndex="${j}"><div class="ivu-table-cell">
<input autocomplete="off" autofocus="autofocus" value="${this.data[i][this.head[j].key]}" onblur="clusterizeTable.options.methods.resetHtml(this,${i},'${this.head[j].key}')" type="text" placeholder="${this.head[j].placeholder}" class="ivu-input ivu-input-default">
                        </div></td>`;
                    }else{
